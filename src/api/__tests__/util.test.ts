...
describe('Util tests', () => {
  ...
  describe('createWorkspace', () => {
    it('creates a new workspace', () => {
      ...
      expect(workspace.buildShipments[0].buildNumber).toBeDefined()
  ...
  describe('updateWorkspace', () => {
    it('updates a workspace', () => {
      const workspace = utils.createWorkspace(testDbString, "Arnav's Shipping")
      workspace.title = "Doss's Shipping"
      utils.updateWorkspace(testDbString, workspace)
      const updatedWorkspace = utils.getWorkspace(testDbString, workspace.id)
      expect(updatedWorkspace.title).toEqual("Doss's Shipping")
    })
  })
})
